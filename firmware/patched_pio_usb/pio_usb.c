...
static void __no_inline_not_in_flash_func(send_pre)(pio_port_t *pp) {
  ...
  // PATCHED: Replace infinite while with bounded loop to prevent freeze
  // See: https://github.com/sekigon-gonnoc/Pico-PIO-USB/issues/192
  for (int _timeout = 0; _timeout < 50000 && (pp->pio_usb_tx->irq & IRQ_TX_EOP_MASK) == 0; _timeout++) {
    tight_loop_contents();
  }
  ...
  // PATCHED: Replace infinite while with bounded loop
  for (int _timeout = 0; _timeout < 50000 && !(pp->pio_usb_tx->fdebug & stall_mask); _timeout++) {
    tight_loop_contents();
  }
...
void __not_in_flash_func(pio_usb_bus_usb_transfer)(pio_port_t *pp,
  ...
  // PATCHED: Replace infinite while with bounded loop to prevent freeze
  // See: https://github.com/sekigon-gonnoc/Pico-PIO-USB/issues/192
  for (int _timeout = 0; _timeout < 50000 && (pp->pio_usb_tx->irq & IRQ_TX_ALL_MASK) == 0; _timeout++) {
    tight_loop_contents();
  }
  pp->pio_usb_tx->irq = IRQ_TX_ALL_MASK; // clear complete flag
  ...
  if (pp->low_speed) {
    ...
    // PATCHED: Replace infinite while with bounded loop to prevent freeze
    for (int _timeout = 0; _timeout < 50000 && *pc <= PIO_USB_TX_ENCODED_DATA_COMP; _timeout++) {
      tight_loop_contents();
    }
  } else {
    // PATCHED: Replace infinite while with bounded loop to prevent freeze
